...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
        {
            fprintf(Output_fp, "================TCB linked list================\n");
            fprintf(Output_fp, "Task   Prev_TCB_addr   TCB_addr   Next_TCB_addr\n\n");
            fclose(Output_fp);
        }
        while(ptcb != 0){
            if (ptcb->OSTCBPrio == 63) {
                printf("%2d     %11x       %6x     %11x  \n", ptcb->OSTCBPrio, ptcb->OSTCBPrev, ptcb, ptcb->OSTCBNext);
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    fprintf(Output_fp, "%2d     %11x       %6x     %11x  \n", ptcb->OSTCBPrio, ptcb->OSTCBPrev, ptcb, ptcb->OSTCBNext);
                    fclose(Output_fp);
                }
                ptcb = ptcb->OSTCBNext;
            }
        ...
        {
            fprintf(Output_fp, "\nTick   Event         CurrentTask ID   NextTask ID   ");
            fprintf(Output_fp, "ResponseTime  #of ContextSwitch  PreemptionTime  OSTimeDly\n");
            fclose(Output_fp);
        }
        ...
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    ...
                    fprintf(Output_fp, "%15d", OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime - OSTCBCur->OSTCBExecutionTime);
        ...
        if (OSTimeGet() >= OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime) {
            if (OSTCBCur->OSTCBWorkTime == OSTCBCur->OSTCBExecutionTime) {
                ...
                if (OSTCBHighRdy->OSTCBPrio == 63) {
                    ...
                    if (OSTCBHighRdy->OSTCBId == OSTCBCur->OSTCBId) {
                        printf("task(%2d)(%2d)%10d", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum + 1, OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                        {
                            fprintf(Output_fp, "task(%2d)(%2d)%10d", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum + 1, OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                            fclose(Output_fp);
                        }
                    }
                    else {
                ...
                printf("%15d", OSTCBCur->OSTCBContexSwitch);
                printf("%15d\n", OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime - OSTCBCur->OSTCBExecutionTime);
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    fprintf(Output_fp, "%15d", OSTCBCur->OSTCBContexSwitch);
                    fprintf(Output_fp, "%15d\n", OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime - OSTCBCur->OSTCBExecutionTime);
                   ...
                   INT16U   ArriveTime)
{
    ...
    if (ptcb != (OS_TCB *)0) {
      ...
      //ptcb->OSTCBDly           = 0u;                     /* Task is not delayed                      */
        ...
        ptcb->OSTCBPeriodic      = 0uL;
        ptcb->OSTCBStartime      = 0uL;
        ...
        if (ptcb->OSTCBPrio == 63) {
            ...
            if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
            {
                ...
                fprintf(Output_fp, "Previous TCB point to address %8x\n", ptcb->OSTCBPrev);
                fprintf(Output_fp, "Current  TCB point to address %8x\n", ptcb);
                fprintf(Output_fp, "Next     TCB point to address %8x\n\n", ptcb->OSTCBNext);
                fclose(Output_fp);
            }
        }
        else {
