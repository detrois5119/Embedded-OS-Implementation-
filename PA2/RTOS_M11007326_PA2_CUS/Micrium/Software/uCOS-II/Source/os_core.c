    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if(OSTCBCur->OSTCBWorkTime != OSTCBCur->OSTCBExecutionTime&& exitflag == 0){
                    OS_SchedNew();
                    OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
                }


                if (OSTCBHighRdy->OSTCBPrio != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    if (exitflag == 0) {
                        ...
                        {
                            fprintf(Output_fp, "%2d     Preemption      ", OSTimeGet());
                            fclose(Output_fp);
                        }
                        ...
                        printf("task(%2d)(%2d)\n", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum);
                        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                        {
                        ...
                        OSTCBCur->OSTCBContexSwitch++;
                        OSTCBHighRdy->OSTCBContexSwitch++;
                    }
                    
                    if (OSTCBHighRdy->OSTCBStartTime == 0 && OSTCBHighRdy->OSTCBWorkTime == 0) {
                        OSTCBHighRdy->OSTCBStartTime = OSTimeGet();
                    }
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        printf("ResponseTime  #of ContextSwitch  PreemptionTime  OSTimeDly\n");
        ...
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur ) {          /* No Ctx Sw if current task is highest rdy     */
...
#endif
#endif
                printf("%2d     Completion      ", OSTimeGet());
                printf("task(%2d)(%2d)   ", OSTCBCur->OSTCBId, OSTCBCur->OSTCBJobNum);
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                ...
                if (OSTCBHighRdy->OSTCBPrio == 63) {
                    printf("task(%2d)%14d", OSTCBHighRdy->OSTCBPrio, OSTimeGet()-OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        fprintf(Output_fp, "task(%2d)%14d", OSTCBHighRdy->OSTCBPrio, OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                        fclose(Output_fp);
                    }
                }
                else {
                    printf("task(%2d)(%2d)", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum);
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        fprintf(Output_fp, "task(%2d)(%2d)", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum);
                        fclose(Output_fp);
                    }
                    if (OSTCBCur->OSTCBCusUti == 0) {
                        printf("%10d", OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                        {
                            fprintf(Output_fp, "%10d", OSTimeGet() - OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum) - OSTCBCur->OSTCBArriveTime);
                            fclose(Output_fp);
                        }
                    }
                    else {
                        printf("%10d", OSTimeGet() - ATaskParameter[completeNumber-1].TaskArriveTime);
                        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                        {
                            fprintf(Output_fp, "%10d", OSTimeGet() - ATaskParameter[completeNumber - 1].TaskArriveTime);
                            fclose(Output_fp);
                        }
                    }
                }
                ...
                printf("%15d", OSTCBCur->OSTCBContexSwitch);
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    fprintf(Output_fp, "%15d", OSTCBCur->OSTCBContexSwitch);
                    fclose(Output_fp);
                }
                
                if (OSTCBCur->OSTCBCusUti == 0) {
                    printf("%15d", OSTimeGet() - OSTCBCur->OSTCBStartTime - OSTCBCur->OSTCBExecutionTime);
                    if (OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime - OSTimeGet() != 0) {
                    ...
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        fprintf(Output_fp, "%15d", OSTimeGet() - OSTCBCur->OSTCBStartTime - OSTCBCur->OSTCBExecutionTime);
                        if (OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime - OSTimeGet() != 0) {
                            fprintf(Output_fp, "%15d\n", OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime - OSTimeGet());
                        }
                        else {
                            fprintf(Output_fp, "\n");
                        }
                    ...
                    printf("%15d", OSTimeGet() - OSTCBCur->OSTCBStartTime - ATaskParameter[completeNumber-1].TaskExecutionTime);
                    printf("            N/A\n");
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        fprintf(Output_fp, "%15d", OSTimeGet() - OSTCBCur->OSTCBStartTime - ATaskParameter[completeNumber-1].TaskExecutionTime);
                        fprintf(Output_fp, "            N/A\n");
                        fclose(Output_fp);
                    }   
                }
                OSTCBCur->OSTCBContexSwitch = 0;
                OSTCBCur->OSTCBJobNum++;
                OSTCBCur->OSTCBWorkTime = 0;
                OSTCBCur->OSTCBStartTime = 0;
                if(OSTCBCur->OSTCBCusUti==0)
                    OSTCBCur->OSTCBDeadLine = OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime;
...
static  void  OS_SchedNew (void)
{
    ...
    ptcb = OSTCBList;                                  /* Point at first TCB in TCB list               */
    hptcb = OSTCBPrioTbl[0];

    while (ptcb != 0) {     /* Go through all TCBs in TCB list              */
        OS_ENTER_CRITICAL();
        int x = ptcb->OSTCBId;
        if (ptcb->OSTCBPrio == 63) {
            ptcb->OSTCBDeadLine = 10000;
        }
        if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY && ptcb->OSTCBDly == 0) {
            if (ptcb->OSTCBDly == 0 && hptcb->OSTCBDly != 0 || ptcb->OSTCBStat!= OS_STAT_RDY) {

                hptcb = ptcb;
            }
            else if (ptcb->OSTCBDeadLine <= hptcb->OSTCBDeadLine && ptcb->OSTCBWorkTime != ptcb->OSTCBExecutionTime) {
                if (ptcb->OSTCBDeadLine == hptcb->OSTCBDeadLine && ptcb->OSTCBId < hptcb->OSTCBId )
                    hptcb = ptcb;
                else if (ptcb->OSTCBDeadLine < hptcb->OSTCBDeadLine)
                    hptcb = ptcb;
            }
            else if (OSTimeGet() == hptcb->OSTCBDeadLine && ptcb->OSTCBDeadLine < hptcb->OSTCBDeadLine + hptcb->OSTCBPeriodic) {
                hptcb = ptcb;
            }
    ...
    OSPrioHighRdy = hptcb->OSTCBPrio;
    if (OSTCBCur != NULL && OSTCBCur->OSTCBPrio != 63) {
        if (OSTCBCur->OSTCBCusUti != 0) {
            if (OSTimeGet() >= ATaskParameter[completeNumber].TaskPeriodic) {
                if (OSTCBCur->OSTCBWorkTime == OSTCBCur->OSTCBExecutionTime) {
                    ...
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                        fprintf(Output_fp, "%2d     MissDeadline    ", OSTimeGet());
                        fprintf(Output_fp, "task(%2d)(%2d)   ------------\n", OSTCBCur->OSTCBId, OSTCBCur->OSTCBJobNum);
                        fclose(Output_fp);
                    }
                    exit(0);
                   ...
                   INT16U   CusUti)
{
    ...
    if (ptcb != (OS_TCB *)0) {
      ...
      //ptcb->OSTCBDly           = 0u;                     /* Task is not delayed                      */
        ...
        ptcb->OSTCBExecutionTime = ExecutionTime;
        ptcb->OSTCBArriveTime    = ArriveTime;
        ptcb->OSTCBPeriodic      = Periodic;
        ptcb->OSTCBCusUti        = CusUti;
#else
        ...
        ptcb->OSTCBDeadLine      = ArriveTime+Periodic;
        ptcb->OSTCBContexSwitch  = 0uL;
        ptcb->OSTCBJobNum        = 0uL;
        ptcb->OSTCBWorkTime      = 0uL;
        ptcb->OSTCBStartTime = 0uL;
#endif
...
void OS_Cusevent (void) {
    ...
    for (int n = completeNumber; n < ATASK_NUMBER; n++) {
        if (OSTimeGet() == ATaskParameter[n].TaskArriveTime) {
            printf("%2d    Aperiodic job(%d) arrives",OSTimeGet(),n);
            if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
            {
                fprintf(Output_fp, "%2d    Aperiodic job(%d) arrives", OSTimeGet(), n);
                fclose(Output_fp);
            }
            if (ptcb->OSTCBStat != OS_STAT_SUSPEND) {
                ...
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    fprintf(Output_fp, " . Do nothing.\n");
                    fclose(Output_fp);
                }
            }
        }
        if (OSTimeGet() >= ATaskParameter[n].TaskArriveTime && ptcb->OSTCBStat == OS_STAT_SUSPEND) {
            ptcb->OSTCBExecutionTime = ATaskParameter[n].TaskExecutionTime;
            if (ptcb->OSTCBDeadLine == 0 ){
                ptcb->OSTCBDeadLine = OSTimeGet() + ptcb->OSTCBExecutionTime / (ptcb->OSTCBCusUti / 100.0);
                printf(" and sets CUS server's deadline as %2d.\n", ptcb->OSTCBDeadLine);
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                {
                    fprintf(Output_fp, " and sets CUS server's deadline as %2d.\n", ptcb->OSTCBDeadLine);
                    fclose(Output_fp);
                }
                ptcb->OSTCBStat = OS_STAT_RDY;
            }
            else if(OSTimeGet() >= ptcb->OSTCBDeadLine && ptcb->OSTCBStat == OS_STAT_SUSPEND){
                if(ATaskParameter[n].TaskArriveTime < OSTimeGet()){
                    ptcb->OSTCBDeadLine = ptcb->OSTCBDeadLine + ptcb->OSTCBExecutionTime / (ptcb->OSTCBCusUti / 100.0);
                    printf("%2d    Aperiodic job(%d) sets CUS server's deadline as %2d.\n",OSTimeGet(),n, ptcb->OSTCBDeadLine);
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
                ...
                else if(ATaskParameter[n].TaskArriveTime == OSTimeGet()){
                    ptcb->OSTCBDeadLine = OSTimeGet() + ptcb->OSTCBExecutionTime / (ptcb->OSTCBCusUti / 100.0);
                    printf(" and sets CUS server's deadline as %2d.\n", ptcb->OSTCBDeadLine);
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                    {
