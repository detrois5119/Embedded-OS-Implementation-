    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur&& OSTCBCur->OSTCBId != OSTCBHighRdy->OSTCBId) {          /* No Ctx Sw if current task is highest rdy */
        ...
        if (OSTimeGet() >= OSTCBCur->OSTCBPeriodic * (OSTCBCur->OSTCBJobNum + 1) + OSTCBCur->OSTCBArriveTime) {
            if (OSTCBCur->OSTCBWorkTime == OSTCBCur->OSTCBExecutionTime) {
                ...
                if (OSTCBHighRdy->OSTCBPrio == 63) {
                    ...
                    if (OSTCBHighRdy->OSTCBId == OSTCBCur->OSTCBId) {
                        printf("task(%2d)(%2d)", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum + 1);
                        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
                        {
                            fprintf(Output_fp, "task(%2d)(%2d)", OSTCBHighRdy->OSTCBId);
                            fclose(Output_fp);
                        }
                    }
                    else {
                   ...
                   INT16U   periodic)
{
    ...
    if (ptcb != (OS_TCB *)0) {
      ...
      //ptcb->OSTCBDly           = 0u;                     /* Task is not delayed                      */
        ...
        ptcb->OSTCBR1Lock        = R1Lock;
        ptcb->OSTCBR1UnLock      = R1UnLock;
        ptcb->OSTCBR2Lock        = R2Lock;
        ptcb->OSTCBR2UnLock      = R2UnLock;
