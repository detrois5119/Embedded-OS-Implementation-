...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (pevent->OSEventName == 1) {
        printf("%2d\ttask %d release R1", OSTimeGet(), OSTCBCur->OSTCBId);
        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
        {
            fprintf(Output_fp, "%2d\ttask %d release R1", OSTimeGet(), OSTCBCur->OSTCBId);
            fclose(Output_fp);
        }
    }
    else {
        printf("%2d\ttask %d release R2", OSTimeGet(), OSTCBCur->OSTCBId);
        if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) == 0)
        {
            fprintf(Output_fp, "%2d\ttask %d release R2", OSTimeGet(), OSTCBCur->OSTCBId);
            fclose(Output_fp);
        }
    }
    if(pcp<=prio){
