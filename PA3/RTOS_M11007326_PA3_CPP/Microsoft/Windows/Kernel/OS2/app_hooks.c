...
void InputFile() {
    ...
    if ((err = fopen_s(&fp, INPUT_FILE_NAME, "r")) == 0)  /*task set 1-4*/
    {
       // printf("The file 'TaskSet.txt' was opened\n");
        ...
        {
      //      fprintf(Output_fp, "The file 'TaskSet.txt' was opened\n");
            fclose(Output_fp);
        }
    }
    else
    ...
    for (int p = 0; p < TASK_NUMBER; p++) {
        if (TaskParameter[p].R1unlock != 0) {
            if (r1max > TaskParameter[p].TaskPriority) {
                r1max = TaskParameter[p].TaskPriority-1;
            }
        }
    }
...
{/*
    if(OSTCBCur->OSTCBPrio==63)
        printf("%d  task( %d)   ", OSTime, OSTCBCur->OSTCBPrio);
    else
        printf("%d  task( %d)( %d)", OSTime, OSTCBCur->OSTCBId, OSTCBCur->OSTCBJobNum);
    OSTCBCur->OSTCBJobNum++;
    if (OSTCBHighRdy->OSTCBPrio == 63)
        printf("    task( %d)       APP_TaskSWHOOK ()\n", OSTCBHighRdy->OSTCBPrio);
    else
        printf("    task( %d)( %d)    APP_TaskSWHOOK ()\n", OSTCBHighRdy->OSTCBId, OSTCBHighRdy->OSTCBJobNum);
*/
